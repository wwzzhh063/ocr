...
class CTC_Model():
    ...
    def base_conv_layer(self, inputs, widths, is_training):
        ...
        with slim.arg_scope([slim.conv2d], kernel_size=[3, 3], weights_regularizer=slim.l2_regularizer(1e-4),
            ...
            with slim.arg_scope([slim.max_pool2d], kernel_size=[2, 2], stride=[2, 1], padding='VALID'):
                ...
                conv9 = slim.conv2d(conv8, 512, scope='conv9', padding='VALID')

                features = tf.squeeze(conv9, axis=1, name='features')
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            if os.path.exists('./model3/ctc.ckpt'.replace('ctc.ckpt', '')):
                saver.restore(sess, './model3/ctc.ckpt')
                print("restore")
            ...
            while True:
                ...
                #
                if i % 100 == 0:
                    saver.save(sess, './model3/ctc.ckpt')
