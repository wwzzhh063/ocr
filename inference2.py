...
def set_xml_data(path,label_path):
    img_path = glob(os.path.join(path, '*'))
    all_img_label  = []             #所有图片的标签

    j = 0
    for img_path in tqdm(img_path):
        img_name = img_path.split('/')[-1].split('.')[0]
        all_bbox_img = Img_ALL_BBox(cv2.imread(img_path))           #一张图片中的所有box   tetst---------------------------------
        all_bbox_img.img_path = img_path

        img_label_path = os.path.join(label_path,img_name)


        for label_xml in glob(os.path.join(img_label_path,'xml/*')):
            bbox_list = (label_xml.split('/')[-1].split('.')[0].split('_')[-1]).replace('[','').replace(']','').split(',')
            bbox_list = list(map(int,bbox_list))
            label, _ = read_label(label_xml)
            bbox = Bbox(bbox_list, label, 'none')
            all_bbox_img.all_box.append(bbox)
        ...
        #             label,name = read_label(small_img_xml_path)
...
def draw_bbox(bbox,img,x_pro,y_pro,color):
    temp1 = (bbox[2]-bbox[0])* x_pro/3
    temp2 = (bbox[3]-bbox[1])* y_pro/7
    cv2.rectangle(img, (int(bbox[0] * x_pro-temp1), int(bbox[1] * y_pro-temp2)),
                  (int(bbox[2] * x_pro+temp1), int(bbox[3] * y_pro+temp2)), color, 4)

def draw_result(img,label,x_pro,y_pro):
    ttfont = ImageFont.truetype('SimSun.ttf',45)
...
if __name__ == '__main__':
    ...
    parser.add_argument('img_path', help='xml path', type=str)
    parser.add_argument('label_path', help='label path', type=str)
    parser.add_argument('save_path', help='label path', type=str)
    args = parser.parse_args()
    all_img_label = set_xml_data(args.img_path,args.label_path)



    for i,img_result in tqdm(enumerate(all_img_label)):
            ...
            if config.ENHANCE:
                for x,cut_img in enumerate(cut_img_list):
                    if len(resize_list) > 0:
                        cut_path = os.path.join(save_path,  '2_'+str(i) + '_' + str(x)+'_' +'0' + '_' + label + '.jpg')
                        ...
                        for j, resize in enumerate(resize_list):
                            try:
                                ...
                                cut_path = os.path.join(save_path,  '2_'+str(i)+'_'+str(x) + '_' + str(j + 1) + '_' + label + '.jpg')
                                cv2.imwrite(cut_path, resize_img)
                            except:
                                pass

                    else:
                        cut_path = os.path.join(save_path, '2_'+ str(i) + '_'+str(x)+'_' + label + '.jpg')
                        cv2.imwrite(cut_path, cut_img)
            else:
                if len(resize_list)>0:
                    cut_path = os.path.join(save_path,  '2_'+str(i) + '_' + '0' + '_' + label + '.jpg')
                    cv2.imwrite(cut_path, cut_img)
                    for j,resize in enumerate(resize_list):
                        resize_img = cv2.resize(cut_img,None,fx=resize[1],fy=resize[0])
                        cut_path = os.path.join(save_path, '2_'+str(i)+'_'+str(j+1)+'_'+label+'.jpg')
                        cv2.imwrite(cut_path,resize_img)

                else:
                    cut_path = os.path.join(save_path,  '2_'+str(i) + '_' + label + '.jpg')
                    cv2.imwrite(cut_path, cut_img)