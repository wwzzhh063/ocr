...
from Arithmetic_Func_detection_for_CTPN_v1.lib.fast_rcnn.config import cfg
from Arithmetic_Func_detection_for_CTPN_v1.lib.fast_rcnn.bbox_transform import bbox_transform_inv, clip_boxes
from Arithmetic_Func_detection_for_CTPN_v1.lib.nms.py_cpu_nms import py_cpu_nms
...
def proposal_layer(rpn_cls_prob_reshape, rpn_bbox_pred, im_info, cfg_key, _feat_stride = [16,], anchor_scales = [16,]):
    ...
    keep = py_cpu_nms(np.hstack((proposals, scores)), nms_thresh)#进行nms操作，保留2000个proposal
    if post_nms_topN > 0:
