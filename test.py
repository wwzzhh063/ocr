...
class CTC_Model():
    ...
    def accuracy(self,pred_decode_result,target_output,label_length):
        ...
        return 1-label_error,1-sequence_error,hypothesis,target_output
    ...
    def build_network(self,inputs,train_output,target_output,sample_rate,is_training,label_length,batch_size):
        ...
        char_acc,seq_acc,hypothesis,target_output = self.accuracy(pred_decode_result,target_output,label_length)
        hypothesis = tf.sparse_to_dense(sparse_indices=hypothesis.indices, output_shape=hypothesis.dense_shape,
                          sparse_values=hypothesis.values, default_value=1)

        target_output = tf.sparse_to_dense(sparse_indices=target_output.indices, output_shape=target_output.dense_shape,
                          sparse_values=target_output.values, default_value=1)


        return char_acc,seq_acc,pred_decode_result,hypothesis,target_output
    ...
    def train(self):
        ...
        char_acc, seq_acc, pred_decode_result, hypothesis, target_output_ = self.build_network(inputs, train_output, target_output,
        ...
        with tf.Session() as sess:
            ...
            val_feedict = {inputs: val_image, train_output: val_labels_input, target_output: val_labels_output,
                           sample_rate: np.min([1., 0.2 * 0 + 0.2]), is_training: False, label_length: label_len,
                           batch_size: [val_image.shape[0]]}

            val_result,target_output_1, val_char_acc, val_seq_acc, = sess.run([pred_decode_result,target_output_, char_acc, seq_acc],
            ...
            train_result = list(
                map(lambda y: ''.join(list(map(lambda x: decode.get(x), y))).split('<EOS>')[0], target_output_1))
            val_result = list(
                map(lambda y: ''.join(list(map(lambda x: decode.get(x), y))).split('<EOS>')[0], val_result))
