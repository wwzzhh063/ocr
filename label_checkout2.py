...
def set_xml_data(path,label_path):
    ...
    for img_path in tqdm(img_path):
        ...
        j,img_label_path = find_labels(img_name,j,label_path)


        for label_xml in glob(os.path.join(img_label_path,'outputs/*')):
...
if __name__ == '__main__':
    ...
    try:
        ...
        for img_label in tqdm(all_img_label):
            ...
            for label in img_label.all_box:
                draw_bbox(label.bbox,img, x_pro, y_pro,(255,0,0))
            ...
            for label in img_label.all_box:
                img = draw_result(img, label, x_pro, y_pro)

            img.save(os.path.join(args.save_path,img_label.img_path.split('/')[-1]))
    except:
        pass
